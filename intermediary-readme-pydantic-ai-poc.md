...
## Architecture documentation (`my_docs/`)

Alongside this migration tracker, the fork keeps a separate doc set under [`my_docs/`](my_docs/). These files are **not** upstream MCP-Universe documentation; they record original design intent, fork-specific evolution, and forensic analysis used to plan and validate migrations on this repo.

| Document | What we track |
|----------|----------------|
| [`my_docs/technical_breakdown.md`](my_docs/technical_breakdown.md) | Baseline upstream architecture: layers, module map, registry pattern, agent / LLM / MCP / evaluator flows |
| [`my_docs/MCP_PLUS.md`](my_docs/MCP_PLUS.md) | MCP+ token-reduction extension: wrapper flow, PostProcessAgent, deployment modes |
