...
from .utils import build_system_prompt, get_iteration_prompt_content
...
class FunctionCallConfig(BaseAgentConfig):
    ...
    Attributes:
        ...
        scheduler_mode (Optional[dict]): Tool call scheduler configuration.
    """
...
class FunctionCallWideResearch(BaseAgent):
    ...
    async def _handle_content_response(
        ...
        try:
            ...
            if "gemini" not in self._llm.config.model_name:
                ...
                messages.append({
                    ...
                    "thought": message_obj.thought,
                    "thought_signature": message_obj.thought_signature
                })
                if len(messages[-1]["content"]) > 0:
                    content = messages[-1]["content"][-1] # take the last part of the content
                else:
                    content = "No function calls and no assistant messages were returned."
    ...
    async def _execute(
        ...
        for iter_num in range(self._config.max_iterations):
            # Build iteration prompt: summarize on last step, or countdown + optional scheduler hint otherwise
            prompt_content = get_iteration_prompt_content(
                iter_num,
            ...
            if hasattr(response, 'choices') and response.choices:
                ...
                if has_tool_calls:
                    ...
                    if has_content and "gemini" not in self._llm.config.model_name:
                        content = message_obj.content.strip()
                        # messages.append({
                        #     "role": "assistant",
                        #     "content": content
                    ...
                    continue
                if has_content:
                    result = await self._handle_content_response(messages, iter_num, callbacks, tracer, message_obj)
    ...
    async def _handle_function_calls(
        ...
        def process_tool_call(tool_call: Any) -> dict:
            formated_tool_call = {
            ...
            if "thought_signature" in tool_call:
                formated_tool_call["thought_signature"] = tool_call["thought_signature"]
            return formated_tool_call
        ...
        if "gemini" in self._llm.config.model_name and hasattr(message_obj, 'content') and message_obj.content:
            assistant_message["content"] = message_obj.content
            assistant_message["thought"] = message_obj.thought
            assistant_message["thought_signature"] = message_obj.thought_signature

        # If message_obj has reasoning_details and it is not None, add to assistant_message
        ...
        for tool_call in tool_calls:
            try:
                ...
                # if model is gemini, add tool_name to the tool_message
                if "gemini" in self._llm.config.model_name:
                    tool_message["tool_name"] = function_name
